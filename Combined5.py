...
past_games = 17          # Past Games to include in Pythagorean Win Projection (must be integer greater than 3) (~17 ?)
...
HFA = 7.2                # home field advantage as percentage, default is 5.5 (~7.2 seems good)
HFA2 = HFA               # home field advantage for win chart
HFAE = 65.0              # elo hfa points
HFA_P_M = 1.73           # multiplier for home field advantage in playoffs (~1.73 seems good)
...
a_factor = 2.45          # Exponential factor to compare Expected Wins Head to Head (~2.45 seems good)
expect_points = 45.4     # Expected total points for calculating point differential (45.4 was 2016 league average points/game)
percent_mine = 40.0      # % using Pwin instead of Elo (must be float) (~40 seems good)
percent_win_chart = 2.51 # % using expected win chart (must be float) (~2.5 seems good)
b_factor = 0.92          # Exponential factor to compare Win Chart values Head to Head (~0.9 seems good)
K = 21.31                # 21.3 better than 20.0?
REVERT = 1/2.99          # 2.99 better than 3.0?
        ...
        for game in games:
            ...
            if game['elo_prob1'] != None:
             #    game['my_prob1'] = .5+my_diff
                if game['playoff'] == 1:
                 ...
                 if game['neutral'] == 1:
                     game['my_prob1'] = (team1['Pwin']) ** a_factor / (
                             (team1['Pwin']) ** a_factor + team2['Pwin'] ** a_factor)
                     game['my_prob2'] = 1.0 / (math.pow(10.0, (-elo_diff / 400.0)) + 1.0)
                     game['my_prob3'] = (game['my_prob1']+game['my_prob2'])/2
                     game['my_prob1'] = (percent_mine / 100) * game['my_prob1'] + (
                             (percent_win_chart / 100) * game['my_prob3']) + (
                                                (1 - ((percent_mine / 100) + (percent_win_chart / 100))) * game[
                                            'my_prob2'])
                     game['point_diff'] = round((expect_points * game['my_prob1']) - (expect_points / 2), 1)
                     game['point_diff2'] = round(
                         (y_factor * (abs(((expect_points * game['my_prob1']) - (expect_points / 2))))) ** (
                                 1 / x_factor), 1)
                     game['my_prob_qb1_out'] = game['my_prob1'] - backup_qb_percent
                     game['my_prob_qb2_out'] = game['my_prob1'] + backup_qb_percent
                     game['point_diff_qb1_out'] = round(
                         (y_factor * (abs(((expect_points * game['my_prob_qb1_out']) - (expect_points / 2))))) ** (
                                 1 / x_factor), 1)
                     game['point_diff_qb2_out'] = round(
                         (y_factor * (abs(((expect_points * game['my_prob_qb2_out']) - (expect_points / 2))))) ** (
                                 1 / x_factor), 1)
                 else:
            ...
            if game['score1'] != None:
              ...
              # Elo shift based on K and the margin of victory multiplier
                shift = (K * mult) * (game['result1'] - game['my_prob2'])
