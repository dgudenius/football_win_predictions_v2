...
class Util2:
    ...
    def read_games(file):
        ...
        file_2018_predict = file.replace(".", "_2018_predict.")
        #urlretrieve("https://projects.fivethirtyeight.com/nfl-api/2018/nfl_games_2018.csv", file_2018)
        games += [item for item in csv.DictReader(open(file_2018_predict))]
    ...
    def evaluate_forecasts(games):
        ...
        if len(upcoming_games) > 0:
            ...
            for game in upcoming_games:
               print("%s\t%s vs. %s\t\t%s%% (Elo)\t\t%s%% (You) Point Diff %s    Point Diff2 %s  Point Diff3 %s    QB1 Out %s and %s QB2 Out %s and %s" % (game['date'], game['team1'], game['team2'], int(round(100 * game['elo_prob1'])),int(round(100 * game['my_prob1'])),game['point_diff'],game['point_diff2'],game['point_diff3'],int(round(100 *game['my_prob_qb1_out'])),game['point_diff_qb1_out'],int(round(100 *game['my_prob_qb2_out'])),game['point_diff_qb2_out']))
